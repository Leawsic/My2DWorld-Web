...
import {CHUNK_SIZE, WORLD_HEIGHT, WORLD_MIN_Y, WORLD_MAX_Y} from "./world";
    ...
    for (let local = 0; local < CHUNK_SIZE; local += 1) {
        ...
        switch (biome.id) {
            ...
            default: {
                ...
                if (rng() < 0.07 && !structuresNear(x, p.seed, 0) && surface + 1 <= WORLD_MAX_Y) setBlock(p, local, surface + 1, p.numFor(FLOWER_KINDS[Math.floor(rng() * FLOWER_KINDS.length)]));
...
function setBlock(p: FeaturePlacement, local: number, y: number, num: number): void {
    ...
    p.blocks[local * WORLD_HEIGHT + (y - WORLD_MIN_Y)] = num;
}
...
function placeTree(p: FeaturePlacement, local: number, surface: number, rng: () => number): void {
    ...
    for (let y = surface + 1; y <= top; y += 1) p.blocks[local * WORLD_HEIGHT + (y - WORLD_MIN_Y)] = logNum;
    ...
    for (let dx = -2; dx <= 2; dx += 1) {
        const lx = local + dx;
        if (lx < 0 || lx >= CHUNK_SIZE || ly > WORLD_MAX_Y) continue;
        if (Math.abs(dx) === 2 && rng() < 0.25) continue;
        setBlock(p, lx, ly, leavesNum);
    }
    ly += 1;
    if (ly <= WORLD_MAX_Y) for (let dx = -1; dx <= 1; dx += 1) setBlock(p, local + dx, ly, leavesNum);
    ly += 1;
    ...
    if (ly + 1 <= WORLD_MAX_Y && rng() < 0.6) setBlock(p, local, ly + 1, leavesNum);
}
...
function placeCactus(p: FeaturePlacement, local: number, surface: number, rng: () => number): void {
    ...
    for (let y = surface + 1; y <= Math.min(surface + height, WORLD_MAX_Y); y += 1) p.blocks[local * WORLD_HEIGHT + (y - WORLD_MIN_Y)] = cactusNum;
}
...
function placeRock(p: FeaturePlacement, local: number, surface: number, rng: () => number): void {
    ...
    if (surface + 1 <= WORLD_MAX_Y) p.blocks[(local + 1) * WORLD_HEIGHT + (surface + 1 - WORLD_MIN_Y)] = stoneNum;
}
