...
function request<T>(url: string, method = "GET", value?: unknown): T {
  ...
  try { xhr.send(value === undefined ? undefined : JSON.stringify(value)); } catch { return null as T; }
  if (xhr.status < 200 || xhr.status >= 300) return null as T;
  try { return xhr.responseText ? JSON.parse(xhr.responseText) as T : (null as T); } catch { return null as T; }
}

export const storage = {
  setUser(username: string): void { activeUser = username || "steve"; },
  account(username: string, password: string, action: "login" | "register"): boolean { return Boolean(request<{ ok?: boolean }>("/api/account", "POST", { username, password, action })?.ok); },
  ...
  loadWorlds(username = "steve"): WorldMeta[] {
    return request<{ worlds?: WorldMeta[] }>(`/api/worlds?user=${encodeURIComponent(username || activeUser)}`)?.worlds || [];
  },
