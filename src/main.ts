...
class GameSession {
  ...
  private bindInput(): void {
    ...
    window.addEventListener("keydown", (event) => {
      ...
      if (event.code === "KeyE" && this.modeName === "creative") { this.toggleInventory(); event.preventDefault(); return; }
      if (this.inventoryOpen) { if (event.key === "Escape") this.toggleInventory(); event.preventDefault(); return; }
      const action = actionFor(event.code);
      if (event.key === "Escape") { this.menu = this.menu ? null : "pause"; this.paused = Boolean(this.menu); if (this.paused) { plugins.notifyGamePause(this.pluginContext()); storage.log("Game paused", { world: this.meta.name }); } else { plugins.notifyGameResume(this.pluginContext()); storage.log("Game resumed", { world: this.meta.name }); } }
  ...
  private handleInventoryClick(clientX: number, clientY: number): void {
    ...
    storage.log("Creative inventory slot changed", { world: this.meta.name, slot: slot.index, row: slot.kind, heldItem: this.heldInventoryItem });
  }
  ...
  private renderCursor(): void {
    if (this.paused || this.chatOpen || this.inventoryOpen) { this.canvas.style.cursor = settings.cursorStyle === "crosshair" ? "crosshair" : "default"; return; }
  ...
  private renderHud(ctx: CanvasRenderingContext2D, width: number, height: number): void {
    ...
    visible.forEach((message, index) => {
      const alpha = this.chatOpen ? 1 : Math.max(0, Math.min(1, (7 - message.age) / 3));
      ctx.globalAlpha = alpha;
