...
class GameSession {
  ...
  private hotbar = ["grass_block_side", "dirt", "stone", "cobblestone", "mossy_cobblestone", "coal_block", "iron_block", "gold_block", "diamond_block"];
  private selected = 0;
  private health = 20;
  ...
  constructor(readonly meta: WorldMeta) {
    ...
    ["grass_block_side", "dirt", "stone", "cobblestone", "mossy_cobblestone", "bedrock", "coal_block", "iron_block", "gold_block", "diamond_block", ...plugins.blocks.keys()].forEach((type) => this.loadBlock(type));
  ...
  private toggleMode(): void { if (this.menu) return; const previousMode = this.modeName; this.modeName = this.modeName === "creative" ? "spectator" : "creative"; this.mode = createMode(this.modeName); this.cameraOffsetX = 0; this.cameraOffsetY = 0; this.dragging = false; this.notice = text("已切换游戏模式", "Game mode switched"); this.noticeTimer = 2; this.save(); plugins.notifyGameModeChanged({ ...this.pluginContext(), previousMode, mode: this.modeName }); storage.log("Game mode changed", { world: this.meta.name, from: previousMode, to: this.modeName }); }
  ...
  private submitChat(input: string): void { this.addChat(`> ${input}`); if (!input.startsWith("/")) { this.addChat(input); return; } const parts = input.slice(1).trim().split(/\s+/); const command = parts[0]?.toLowerCase(); if (command === "gamemode" && (parts[1] === "creative" || parts[1] === "spectator")) { const previousMode = this.modeName; this.modeName = parts[1]; this.mode = createMode(this.modeName); this.save(); plugins.notifyGameModeChanged({ ...this.pluginContext(), previousMode, mode: this.modeName }); storage.log("Game mode changed", { world: this.meta.name, from: previousMode, to: this.modeName }); this.addChat(`Gamemode set to ${this.modeName}`); } else if ((command === "speed" || command === "movespeed") && Number.isFinite(Number(parts[1]))) { const speed = Math.max(0.1, Math.min(50, Number(parts[1]))); this.player.movement.walkSpeed = speed; this.meta.physics.walkSpeed = speed; settings.movement.walkSpeed = speed; storage.saveSettings(settings); const worlds = storage.loadWorlds(username).map((world) => world.id === this.meta.id ? this.meta : world); storage.saveWorlds(worlds, username); this.save(); storage.log("Movement speed changed", { world: this.meta.name, speed }); this.addChat(`Movement speed set to ${speed}`); } else if (command === "debug" && ["on", "off", "true", "false"].includes(parts[1])) { this.debug = parts[1] === "on" || parts[1] === "true"; settings.debugDefault = this.debug; storage.saveSettings(settings); this.addChat(`Debug ${this.debug ? "on" : "off"}`); } else this.addChat("Unknown or invalid command"); }
  ...
  private render(): void {
    const width = window.innerWidth; const height = window.innerHeight; const cameraX = this.player.x + this.cameraOffsetX; const cameraY = this.player.y + this.cameraOffsetY; const ctx = this.ctx;
    ctx.fillStyle = "#82c9d7"; ctx.fillRect(0, 0, width, height);
    const left = Math.floor(cameraX - width / this.blockSize / 2 - 1); const right = Math.ceil(cameraX + width / this.blockSize / 2 + 1); const bottom = Math.max(1, Math.floor(cameraY - height / this.blockSize / 2 - 1)); const top = Math.ceil(cameraY + height / this.blockSize / 2 + 1);
    for (const [chunkX, chunk] of this.world.chunks) { if (chunkX * 16 > right || (chunkX + 1) * 16 < left) continue; for (let x = Math.max(left, chunk.start); x < Math.min(right, chunk.start + 16); x += 1) for (let y = bottom; y <= Math.min(top, chunk.surfaces.get(x) ?? 0); y += 1) { const type = this.world.getBlock(x, y); if (!type) continue; const sx = Math.round((x - cameraX) * this.blockSize + width / 2); const sy = Math.round((cameraY - y) * this.blockSize + height / 2); const image = this.blockImages.get(type); if (image?.complete && image.naturalWidth) ctx.drawImage(image, sx, sy, this.blockSize, this.blockSize); else { const palette: Record<string, string> = { grass_block_side: "#62a941", dirt: "#8d613c", stone: "#777d82", cobblestone: "#626b6d", mossy_cobblestone: "#4c7564", bedrock: "#303940" }; ctx.fillStyle = palette[type] ?? "#cc39b7"; ctx.fillRect(sx, sy, this.blockSize, this.blockSize); } } }
    for (const [cell, type] of this.world.placedBlocks) { const [x, y] = World.parseCell(cell); if (x < left || x > right || y < bottom || y > top) continue; const sx = Math.round((x - cameraX) * this.blockSize + width / 2); const sy = Math.round((cameraY - y) * this.blockSize + height / 2); const image = this.blockImages.get(type); if (image?.complete && image.naturalWidth) ctx.drawImage(image, sx, sy, this.blockSize, this.blockSize); else { ctx.fillStyle = "#cc39b7"; ctx.fillRect(sx, sy, this.blockSize, this.blockSize); } }
